...
const StudentDashboard = () => {
  ...
  const handleAddClass = () => {
    ...
    setTimetable((prev) => ({
      ...prev,
      [editingDay]: [...(prev[editingDay] || []), { ...newClass, id: Date.now() }].sort(
  ...
  const todayClasses = (timetable[todayKey] || []).sort(
    (a, b) => parseTimeToMinutes(a.startTime) - parseTimeToMinutes(b.startTime),
  );
    ...
    <div className="min-h-screen bg-white pb-16">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="relative flex-shrink-0">
                {displayPhoto ? (
                  <img
                    src={displayPhoto}
                    alt="Profile"
                    className="w-14 h-14 rounded-full object-cover ring-2 ring-white/30 ring-offset-2 ring-offset-slate-900"
                  />
                ) : (
            ...
            <div className="flex items-center gap-2.5 flex-shrink-0">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-accent text-white font-bold text-sm shadow-[0_0_20px_rgba(255,107,53,0.3)] hover:opacity-90 transition-opacity"
              >
                Edit Profile
              </button>
            </div>
          ...
          {nextClassToday && (
            <div className="mt-4 bg-white/10 border border-white/20 rounded-xl px-5 py-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-xl bg-cyan-400/20 flex items-center justify-center text-xl flex-shrink-0">
                  ⏰
                </div>
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-white font-semibold truncate mt-0.5">
                    {nextClassToday.subject} • {nextClassToday.startTime}
                    {nextClassToday.location ? ` • ${nextClassToday.location}` : ""}
                  </p>
                </div>
              </div>
              <button
                onClick={() => setSelectedDay(todayKey)}
                className="text-sm text-cyan-400 font-semibold hover:text-cyan-300 transition-colors whitespace-nowrap"
              >
                View →
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {serviceModules.map((service) => (
            ...
            >
              ...
              <p className="text-[11px] text-primary/70 font-semibold mt-0.5 uppercase tracking-wide">
                {service.subtitle}
              </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {loadingActivity ? (
              ...
              <div className="space-y-2">
                {activityFeed.slice(0, 4).map((activity) => (
                  ...
                  >
                    <span className="w-9 h-9 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-lg flex-shrink-0">
                      {activity.icon}
                    </span>
          ...
          <div className="lg:col-span-5 bg-white border-2 border-primary/20 rounded-2xl p-6 shadow-sm ring-1 ring-primary/10">
            <div className="flex items-center justify-between mb-5">
              ...
              <span className="text-[10px] text-white bg-primary rounded-lg px-2.5 py-1 font-bold uppercase tracking-wider">
                Admin
              </span>
            </div>
            ...
            {loadingNotices ? (
              ...
              <div className="text-center py-8">
                <div className="w-12 h-12 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center text-2xl mx-auto mb-3">
                  📋
                </div>
              ...
              <div className="space-y-3">
                {notices.map((notice) => (
                  <div
                    key={notice._id}
                    className="rounded-xl bg-primary/5 border border-primary/15 p-3.5 hover:border-primary/40 hover:bg-primary/10 transition-all"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-sm font-semibold text-slate-900 leading-snug">{notice.title}</p>
                      {notice.priority === "high" && (
                        <span className="text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded-md bg-red-500/15 border border-red-500/30 text-red-500 flex-shrink-0">
                          Urgent
                        </span>
                      )}
                    </div>
      ...
      >
        <div className="bg-gradient-to-br from-primary/5 via-white to-accent/5 border-2 border-primary/20 rounded-2xl p-6 shadow-sm ring-1 ring-accent/20">
          ...
          {(() => {
            const dayClasses = (timetable[selectedDay] || []).sort(
              (a, b) => parseTimeToMinutes(a.startTime) - parseTimeToMinutes(b.startTime),
            );
            ...
            if (dayClasses.length === 0) {
              return (
                <div className="flex flex-col items-center justify-center py-10 text-center">
                  <div className="w-14 h-14 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center text-2xl mb-3">
                    📅
                  </div>
                  <p className="text-slate-500 font-semibold">No classes on {DAY_FULL[selectedDay]}</p>
                  <p className="text-slate-400 text-sm mt-1">
                    Click <span className="text-accent font-bold">+ Edit</span> to add your schedule
                  </p>
                </div>
              );
            }
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        {isNext && (
                          <span className="text-[10px] bg-accent text-white px-2 py-0.5 rounded-lg font-black tracking-wider animate-pulse">
                            NEXT
                          </span>
                        )}
                      </div>
                      ...
                      <div
                        className={`mt-3 h-px w-8 ${isNext ? "bg-accent" : "bg-slate-600"} group-hover:w-full transition-all duration-500 rounded-full`}
                      ></div>
                    </div>
                  );
                })}
      ...
      <div
        className={`fixed inset-0 z-[60] transition-opacity duration-300 ${isTimetableEditorOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      >
        <div
          className="absolute inset-0 bg-black/60 backdrop-blur-sm"
          onClick={() => setIsTimetableEditorOpen(false)}
        ></div>
        <div
          className={`absolute top-0 right-0 h-full w-full max-w-md bg-slate-900 border-l border-slate-800 shadow-2xl p-6 overflow-y-auto transform transition-transform duration-300 ease-out ${isTimetableEditorOpen ? "translate-x-0" : "translate-x-full"}`}
        >
          ...
          <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-800">
            ...
            <button
              onClick={() => setIsTimetableEditorOpen(false)}
              className="w-8 h-8 rounded-lg bg-slate-800 border border-slate-700 text-slate-400 hover:text-white hover:bg-slate-700 transition flex items-center justify-center"
            >
              ✕
            </button>
          </div>
          ...
          <div className="mb-5">
            <p className="text-xs text-slate-400 uppercase tracking-widest font-bold mb-3">
              {DAY_FULL[editingDay]} Classes
            </p>
            {(timetable[editingDay] || []).length === 0 ? (
              <p className="text-slate-600 text-sm py-3">No classes added yet.</p>
            ) : (
              <div className="space-y-2">
                {(timetable[editingDay] || [])
                  .sort((a, b) => parseTimeToMinutes(a.startTime) - parseTimeToMinutes(b.startTime))
                  .map((cls) => (
                    <div
                      key={cls.id}
                      className="flex items-center justify-between p-3 rounded-xl bg-slate-800 border border-slate-700 group"
                    >
                      <div className="flex-1 min-w-0">
          ...
          <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4 space-y-3">
            ...
            {classError && (
              <p className="text-xs text-red-400 bg-red-500/10 border border-red-400/20 rounded-lg px-3 py-2">
                {classError}
              </p>
            )}
      ...
      {/* Profile Drawer */}
      <div
        className={`fixed inset-0 z-[60] transition-opacity duration-300 ${isProfileDrawerOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      >
        <div
        ...
        ></div>
        <div
          className={`absolute top-0 right-0 h-full w-full max-w-md bg-slate-900 border-l border-slate-800 shadow-2xl p-6 overflow-y-auto transform transition-transform duration-300 ease-out ${isProfileDrawerOpen ? "translate-x-0" : "translate-x-full"}`}
        >
          ...
          <div className="relative flex items-center justify-between mb-6 pb-4 border-b border-slate-800">
            <div>
              ...
              <h2 className="text-2xl font-extrabold bg-gradient-to-r from-primary to-cyan-400 bg-clip-text text-transparent mt-0.5">
                Edit Profile
              </h2>
            </div>
            <button
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-2">
                  <input
                    ...
                    className="text-sm text-slate-300 file:mr-3 file:px-3 file:py-2 file:rounded-lg file:border-0 file:bg-primary/20 file:text-primary file:font-semibold hover:file:bg-primary/30"
                  />
                  <button
                    type="button"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full mt-1.5 px-3 py-2.5 rounded-xl border border-slate-700 bg-slate-800 focus:ring-2 focus:ring-primary/30 focus:border-primary/40 outline-none transition text-white"
                />
              </div>
              ...
              <div className="flex items-center gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-accent text-white font-bold hover:opacity-90 transition disabled:opacity-50 text-sm shadow-[0_0_20px_rgba(255,107,53,0.3)]"
                >
                ...
                <button
                  ...
                  className="px-3 py-2.5 rounded-xl bg-red-500/10 text-red-400 border border-red-500/20 font-semibold hover:bg-red-500/20 transition text-sm"
                >
                  Delete
                </button>
              </div>
