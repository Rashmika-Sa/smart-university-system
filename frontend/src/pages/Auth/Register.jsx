...
// ── shared stepper config (student tab) ──────────────────────────────────────
const STEPS = [
  { num: 1, label: "Email" },
  { num: 2, label: "Verify" },
  { num: 3, label: "Profile" },
...
// ── Student tab ───────────────────────────────────────────────────────────────
const StudentInputField = ({ label, icon, type = "text", placeholder, value, onChange, children, ...rest }) => (
  <div className="space-y-1.5">
    ...
    <div className="relative group">
      <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
        <svg className="w-4 h-4 text-slate-500 group-focus-within:text-cyan-400 transition-colors"
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl p-8">
          ...
          return (
            <div key={num} className="flex flex-col items-center gap-1.5 z-10">
              ...
              <span className={`text-[10px] font-semibold uppercase tracking-wider transition-colors
                ${current ? "text-cyan-400" : done ? "text-slate-500" : "text-slate-700"}`}>
                {label}
              </span>
            </div>
          );
      ...
      {step === 1 && (
        <form onSubmit={handleSendCode} className="space-y-5">
          ...
          <button disabled={loading}
            className="relative w-full overflow-hidden py-3.5 rounded-xl font-bold text-sm text-white bg-accent shadow-[0_0_30px_rgba(255,107,53,0.35)] hover:shadow-[0_0_45px_rgba(255,107,53,0.55)] transition-all duration-300 hover:-translate-y-0.5 disabled:opacity-60 group">
      ...
      {step === 2 && (
        <form onSubmit={handleVerifyCode} className="space-y-5">
          ...
          <button type="button" onClick={() => { setStep(1); setError(""); setMsg(""); }}
            className="w-full text-slate-600 text-sm hover:text-cyan-400 transition-colors">
            ← Back to Email
          </button>
        </form>
      )}
      ...
      {step === 3 && (
        <form onSubmit={handleRegister} className="space-y-5">
          <StudentInputField label="Full Name"
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative group">
              ...
              <input type={showConfirmPass ? "text" : "password"} placeholder="Re-enter your password"
                ...
                className={`w-full pl-11 pr-12 py-3.5 rounded-xl bg-white/5 border text-white placeholder-slate-600 text-sm focus:ring-2 focus:ring-cyan-400/40 focus:border-cyan-400/40 outline-none transition-all duration-200
                  ${confirmPassword && confirmPassword !== password ? "border-red-500/50"
                    : confirmPassword && confirmPassword === password ? "border-cyan-400/50"
                    : "border-white/10"}`} />
            ...
            {confirmPassword && confirmPassword !== password && <p className="text-xs text-red-400">Passwords do not match</p>}
            {confirmPassword && confirmPassword === password && <p className="text-xs text-cyan-400">Passwords match</p>}
          </div>

          <button disabled={loading}
            className="relative w-full overflow-hidden py-3.5 rounded-xl font-bold text-sm text-white bg-gradient-to-r from-cyan-500 to-indigo-500 shadow-[0_0_30px_rgba(34,211,238,0.35)] hover:shadow-[0_0_45px_rgba(34,211,238,0.55)] transition-all duration-300 hover:-translate-y-0.5 disabled:opacity-60 group">
            <span className="relative z-10 flex items-center justify-center gap-2">
              {loading ? (
      ...
      <p className="text-center text-sm text-slate-500">
        Already enrolled?{" "}
        <Link to="/login" className="text-cyan-400 font-semibold hover:text-cyan-300 transition-colors">Sign In</Link>
      </p>
    </div>
  );
};
...
// ── Managing Body tab ─────────────────────────────────────────────────────────
const MbField = ({ label, type = "text", placeholder, value, onChange, required, children }) => (
  <div className="space-y-1.5">
    <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{label}</label>
    <div className="relative">
      <input type={type} placeholder={placeholder} value={value} onChange={onChange} required={required}
        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-600 text-sm focus:ring-2 focus:ring-cyan-400/40 focus:border-cyan-400/40 outline-none transition-all duration-200" />
      {children}
    </div>
  </div>
);

const ManagingBodySuccessScreen = ({ onReset }) => (
  <div className="text-center py-6">
    ...
    <p className="text-slate-400 text-sm leading-relaxed mb-6">
      Your registration is under review by the Sports Council and Facility Admin.<br />
      You'll receive an email once approved.
    </p>
    <Link to="/login"
      className="inline-block px-6 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-500 text-white text-sm font-semibold hover:opacity-90 transition-opacity">
      Back to Login
    </Link>
  </div>
);

const ManagingBodyTab = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (role === "team_captain") { payload.teamName = teamName; payload.sportName = sportName; }
      else { payload.societyName = societyName; }
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl p-8">
      {success ? <ManagingBodySuccessScreen /> : (
        <>
          <div className="h-px w-full bg-gradient-to-r from-transparent via-cyan-400/40 to-transparent mb-6" />

          {/* role selector */}
          <div className="flex rounded-xl border border-white/10 bg-white/5 p-1 mb-7">
            {[{ value: "team_captain", label: "Team Captain" }, { value: "society", label: "Society" }].map((opt) => (
              <button key={opt.value} type="button"
                onClick={() => { setRole(opt.value); setError(""); }}
                className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all duration-200
                  ${role === opt.value ? "bg-indigo-500 text-white shadow-sm" : "text-slate-500 hover:text-slate-300"}`}>
                {opt.label}
              </button>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <MbField label="Full Name" placeholder="e.g. Alexander Hamilton"
              value={name} onChange={(e) => setName(e.target.value)} required />

            <MbField label="University Email" type="email" placeholder="itXXXXXXXX@my.sliit.lk"
              value={email} onChange={(e) => setEmail(e.target.value)} required />

            {role === "team_captain" ? (
              <>
                <MbField label="Team Name" placeholder="e.g. SLIIT Volleyball Team A"
                  value={teamName} onChange={(e) => setTeamName(e.target.value)} required />
                <MbField label="Sport" placeholder="e.g. Volleyball"
                  value={sportName} onChange={(e) => setSportName(e.target.value)} required />
              </>
            ) : (
              <MbField label="Society Name" placeholder="e.g. SLIIT Drama Society"
                value={societyName} onChange={(e) => setSocietyName(e.target.value)} required />
            )}

            <MbField label="Password" type={showPass ? "text" : "password"}
...
const TABS = [
  ...
  { key: "managing_body", label: "Managing Body" },
];
    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-4">
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <img src="/sliit-official-logo.png" alt="SLIIT Logo" className="h-14 mx-auto mb-4 object-contain" />
          ...
          <p className="text-slate-500 text-sm mt-1 font-mono tracking-widest uppercase">
            <span className="bg-gradient-to-r from-indigo-500 to-cyan-500 bg-clip-text text-transparent font-bold">
              SLIIT Smart University Portal
            </span>
          </p>
        </div>
        ...
        {/* Tab content */}
        {activeTab === "student" ? <StudentTab /> : <ManagingBodyTab />}

        <p className="text-center text-slate-400 text-xs mt-6 font-mono">SLIIT · SMART UNIVERSITY SYSTEM · v2.0</p>
      </div>
    </div>
  );
};
