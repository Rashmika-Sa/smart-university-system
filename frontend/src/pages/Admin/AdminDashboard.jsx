...
const AdminDashboard = () => {
  ...
  return (
    <div className="flex h-screen bg-white font-sans text-gray-800 relative overflow-hidden">
      {/* --- SIDEBAR --- */}
      <div
        className={`${isSidebarOpen ? "w-64" : "w-20"} bg-slate-900 border-r border-slate-800 text-slate-400 transition-all duration-300 flex flex-col shadow-xl relative z-10`}
      >
        {/* Sidebar Header */}
        <div className="p-5 flex items-center justify-between font-bold border-b border-slate-800">
          {isSidebarOpen && (
            <div className="flex items-center gap-2">
              <img src="/sliit-official-logo.png" alt="SLIIT Logo" className="h-8 w-auto object-contain" />
        ...
        <div className="p-4 border-t border-slate-800">
          <button
            ...
            className="flex items-center gap-3 w-full p-3 hover:bg-red-500/10 rounded-xl transition text-left text-slate-500 hover:text-red-400"
          >
            <FaSignOutAlt />
...
const SidebarItem = ({ icon, text, active, onClick, expanded }) => (
  ...
  >
    <span className={`text-xl ${active ? "text-white" : "group-hover:text-accent"}`}>{icon}</span>
