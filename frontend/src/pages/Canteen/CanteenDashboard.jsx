             ...
             <div>
                ...
                {isSuperAdmin ? (
                    <div className="relative" ref={dropdownRef}>
                        <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-3 text-white text-xl font-bold hover:text-indigo-400 transition-colors">
                            {selectedCanteen}
                            <svg className={`w-4 h-4 text-indigo-400 transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : 'rotate-0'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                        </button>
                        {isDropdownOpen && (
                            <div className="absolute top-full left-0 mt-4 w-64 bg-slate-800 rounded-xl border border-slate-700 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                {canteenOptions.map((canteen) => (
                                    <button key={canteen} onClick={() => { safeSetSelectedCanteen(canteen); setIsDropdownOpen(false); }} className={`w-full text-left px-5 py-3 text-sm font-semibold hover:bg-slate-700 ${selectedCanteen === canteen ? 'bg-accent text-white' : 'text-slate-300'}`}>
                                        {canteen}
                                    </button>
                                ))}
                            </div>
        ...
        {activeTab === 'orders' && (
            <div>
                 <div className="flex justify-between items-end mb-6">
                    <div>
                        <h2 className="text-3xl font-bold text-white tracking-tight">Live Orders</h2>
                        <p className="text-slate-400 mt-1">Real-time order feed. Auto-refreshes every 15s.</p>
                    </div>
                    <div className="flex gap-2">
                         <button onClick={() => fetchOrders(selectedCanteen, true)} className="flex items-center gap-2 px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg text-sm font-bold text-slate-300 hover:bg-slate-800 hover:text-white transition">
                    ...
                    <div className="flex flex-col items-center justify-center py-24 text-slate-400">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {orders.filter(o => !['Completed', 'Cancelled'].includes(o.status)).length === 0 && (
                            <div className="col-span-full text-center py-24 bg-slate-900 rounded-3xl border border-dashed border-slate-700">
                                <div className="text-5xl mb-4 opacity-50">😴</div>
                                <h3 className="text-xl font-bold text-white">No Active Orders</h3>
                                <p className="text-slate-400">The kitchen is quiet.</p>
                            </div>
                        )}
                        ...
                        {orders.map((order) => {
                             ...
                             return (
                                <div key={order._id} className={`bg-slate-900 rounded-2xl shadow-sm border border-slate-800 overflow-hidden flex flex-col transition-all duration-300 ${order.status === 'Pending' ? 'ring-2 ring-accent shadow-accent/20' : ''}`}>
                                    
                                    {/* Header Section */}
                                    <div className="px-5 py-4 border-b border-slate-800 flex flex-col bg-slate-800/50 gap-3">
                                        ...
                                        <div className="bg-slate-800 p-3 rounded-xl border border-slate-700 shadow-sm flex flex-col gap-1 mt-1">
                                            ...
                                            <div className="flex items-center gap-2 pl-7">
                                                <span className="text-xs font-bold bg-blue-500/15 text-blue-400 px-2 py-1 rounded-md tracking-wide border border-blue-500/30">
                                        ...
                                        {(order.items || []).map((item, idx) => (
                                            <div key={idx} className="flex justify-between items-start text-sm">
                                                <div className="flex gap-3">
                                                    <span className="font-bold text-slate-500 bg-slate-800 px-2 rounded-md h-6 flex items-center">{item.qty || item.quantity}x</span>
        ...
        {activeTab === 'menu' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="lg:col-span-4 xl:col-span-3">
                    <div className="bg-slate-900 rounded-2xl shadow-xl overflow-hidden sticky top-28 border border-slate-800">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            <div className="space-y-1.5">
                                ...
                                <input type="text" name="name" placeholder="e.g. Chicken Kotto" value={newItem.name} onChange={handleChange} className="w-full px-4 py-3 rounded-lg bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:ring-2 focus:ring-accent outline-none transition font-medium" required />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Price (LKR)</label>
                                    <input type="number" name="price" placeholder="0.00" value={newItem.price} onChange={handleChange} className="w-full px-4 py-3 rounded-lg bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:ring-2 focus:ring-accent outline-none transition font-medium" required />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Category</label>
                                    <div className="relative">
                                        <select name="category" value={newItem.category} onChange={handleChange} className="w-full px-4 py-3 rounded-lg bg-slate-800 border border-slate-700 text-white focus:ring-2 focus:ring-accent outline-none transition font-medium appearance-none cursor-pointer">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <input type="text" name="image" placeholder="https://..." value={newItem.image} onChange={handleChange} className="w-full px-4 py-3 rounded-lg bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:ring-2 focus:ring-accent outline-none transition text-sm" />
                            </div>
                ...
                <div className="lg:col-span-8 xl:col-span-9">
                    ...
                    <div className="mb-6 relative group max-w-md">
                        ...
                        <input type="text" placeholder="Search menu..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-900 text-white border border-slate-800 focus:ring-2 focus:ring-accent/30 focus:border-accent outline-none transition shadow-sm placeholder-slate-500" />
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {filteredItems.map((item) => (
                            <div key={item._id} className={`bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-800 hover:shadow-xl hover:border-slate-700 transition-all duration-300 group flex flex-col relative overflow-hidden ${!item.isAvailable ? 'opacity-80 grayscale-[0.5]' : ''}`}>
                                ...
                                <div className="flex gap-4 mb-4 pl-3">
                                    <div className="w-20 h-20 bg-slate-800 rounded-xl flex-shrink-0 flex items-center justify-center text-3xl overflow-hidden relative shadow-inner border border-slate-700">
                                        {item.image ? <img src={item.image} alt={item.name} className="w-full h-full object-cover" /> : <span>{item.category === 'Rice' ? '🍛' : '🍔'}</span>}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h3 className="font-bold text-white leading-tight mb-1 truncate">{item.name}</h3>
                                        <p className="text-xs text-slate-400 font-bold bg-slate-800 inline-block px-2 py-0.5 rounded-full mb-2 uppercase tracking-wide">{item.category}</p>
                                        <p className="text-lg font-bold text-accent">LKR {item.price}</p>
                                    </div>
                                </div>
                                <div className="mt-auto pl-3 flex items-center justify-between border-t border-slate-800 pt-3">
