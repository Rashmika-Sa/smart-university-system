...
// ─────────────────────────────────────────────────────────────────────────────
//  Leaflet / OpenStreetMap loader  (free — no API key)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  ADVANCED 3D SEAT MAP  (full implementation from v1)
// ─────────────────────────────────────────────────────────────────────────────
...
const I = {
  ...
  close:  'M6 18L18 6M6 6l12 12', check: 'M5 13l4 4L19 7',
  ...
  search: 'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0', arrow: 'M13 7l5 5m0 0l-5 5m5-5H6',
  back:   'M11 17l-5-5m0 0l5-5m-5 5h12', info: 'M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
...
// ─────────────────────────────────────────────────────────────────────────────
//  SCHEDULE CARD — integrated OSM map with Book button UNDER the map
// ─────────────────────────────────────────────────────────────────────────────
const ScheduleCard = ({ schedule, onBook, hasBooked, dark }) => {
  ...
  const mapDivCallback = useCallback(async (el) => {
    ...
    // Check if element exists and map hasn't been initialized
    if (!el || mapInitedRef.current) return;
    
    // Additional check - if map already exists in this container, clean it up first
    if (mapRef.current) {
    ...
    if (el._leaflet_id) {
      console.log('Map container already has Leaflet ID, skipping initialization');
      return;
    }
    ...
    try {
      ...
      // Ignore stale async inits (e.g., StrictMode remounts)
      if (reqId !== mapInitReqRef.current || !el.isConnected) {
        setMapLoading(false);
      ...
      if (!hasCoords) {
        setMapError('No map coordinates saved for this route. Ask the shuttle admin to update it using the map picker.');
        setMapLoading(false);
        mapInitedRef.current = false; // Reset on error
        return;
      }
      ...
      // If a previous map left a stale Leaflet container marker, reset it.
      if (el._leaflet_id) {
        delete el._leaflet_id;
        el.innerHTML = '';
      }

      const map = L.map(el, { zoomControl: true, scrollWheelZoom: false }).setView([centerLat, centerLng], 12);
      mapRef.current = map;
      ...
      const greenIcon = L.divIcon({
        className: '',
        html: `<div style="width:18px;height:18px;border-radius:50%;background:#10b981;border:3px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.35)"></div>`,
        iconSize: [18, 18], iconAnchor: [9, 9],
      });
      const redIcon = L.divIcon({
        className: '',
        html: `<div style="width:18px;height:18px;border-radius:50%;background:#ef4444;border:3px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.35)"></div>`,
        iconSize: [18, 18], iconAnchor: [9, 9],
      });

      L.marker([startLat, startLng], { icon: greenIcon }).addTo(map)
        .bindPopup(`<b style="font-size:12px">🟢 Start</b><br><span style="font-size:11px">${startPoint}</span>`, { closeButton: false })
        .openPopup();

      L.marker([endLat, endLng], { icon: redIcon }).addTo(map)
        .bindPopup(`<b style="font-size:12px">🔴 End</b><br><span style="font-size:11px">${endPoint}</span>`, { closeButton: false });

      L.polyline([[startLat, startLng], [endLat, endLng]], {
        color: '#1B4D89', weight: 3, opacity: 0.8, dashArray: '8 6',
      }).addTo(map);
  ...
  // Cleanup map when component unmounts OR when schedule changes
  useEffect(() => {
    return () => {
      // Invalidate any in-flight async init before tearing down.
  ...
  }, [schedule._id]); // Add schedule._id as dependency to cleanup when schedule changes
    ...
    <div className={`relative overflow-hidden rounded-2xl border transition-all duration-300 hover:-translate-y-0.5 hover:shadow-2xl
      ...
      <div className="p-5">
        ...
        <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl mb-4 text-xs font-semibold ${dark ? 'bg-white/5' : 'bg-[#F5F7FA]'}`}>
          ...
          <div className="flex-1 flex items-center gap-0.5">{[...Array(4)].map((_, i) => <div key={i} className={`flex-1 h-px ${dark ? 'bg-white/10' : 'bg-gray-200'}`} />)}<Ic d={I.arrow} className="w-3 h-3 text-[#1B4D89] shrink-0" /></div>
      ...
      {/* ── Route Map Section (displayed directly, no dropdown) ── */}
      <div className="border-t border-[#E0E4EB] dark:border-white/10">
        <div className="px-5 py-3 bg-[#F5F7FA] dark:bg-white/5">
          <div className="flex items-center gap-2 text-xs font-semibold text-[#6B7280] dark:text-[#6B7280] mb-2">
            ...
            <span>Route Map</span>
            {hasCoords && <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-500/15 text-emerald-400">📍 Live Location</span>}
            {!hasCoords && <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-[#FF6B35]/15 text-[#FF6B35]">⚠ No coordinates</span>}
          </div>
          ...
          {mapLoading && (
            <div className="flex flex-col items-center justify-center gap-3" style={{ height: 220 }}>
              ...
              <p className="text-xs text-[#6B7280] font-medium">Loading route map…</p>
            </div>
          )}
          
          {mapError && !mapLoading && (
            <div className="flex flex-col items-center justify-center gap-2 py-14 text-center px-6 bg-[#F5F7FA] dark:bg-gray-900 rounded-xl">
          ...
          <div ref={mapDivCallback} style={{ height: 220, width: '100%', display: mapError ? 'none' : 'block' }} />
          
          {!mapLoading && !mapError && hasCoords && (
            <div className="flex items-center justify-between mt-2 text-[10px] font-semibold text-[#6B7280]">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-400 inline-block"></span><span className="truncate max-w-[120px]">{startPoint}</span></div>
                <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-rose-400 inline-block"></span><span className="truncate max-w-[120px]">{endPoint}</span></div>
              </div>
              <div className="flex items-center gap-1">🗺 OpenStreetMap</div>
            </div>
          )}
        </div>
      </div>

      {/* ── Book Seat Button (MOVED UNDER THE MAP) ── */}
...
export default function StudentShuttleDashboard() {
  ...
  const [dark, setDark] = useState(false); const [tab, setTab] = useState('schedules');
  const [toast, setToast] = useState(null); const [loading, setLoading] = useState(false);
  const [search, setSearch] = useState(''); const [schedules, setSchedules] = useState([]);
  const [myBookings, setMyBookings] = useState([]); const [takenSeats, setTakenSeats] = useState([]);
  const [bookingSchedule, setBookingSchedule] = useState(null); const [selectedSeat, setSelectedSeat] = useState(null);
  const [confirming, setConfirming] = useState(false); const [is3DVisible, setIs3DVisible] = useState(false);
  const prevBookingsRef = useRef([]); const [newlyConfirmed, setNewlyConfirmed] = useState([]);
  ...
  const openBooking = async schedule => {
    ...
    await fetchTakenSeats(schedule._id); setTimeout(() => setIs3DVisible(true), 150);
  };
  ...
  const D = dark, bg = D ? 'bg-gray-950' : 'bg-[#F5F7FA]', surface = D ? 'bg-gray-900' : 'bg-white';
  const border = D ? 'border-white/10' : 'border-[#E0E4EB]', text = D ? 'text-white' : 'text-gray-900', muted = 'text-[#6B7280]';
  const inputCls = D ? 'bg-white/5 border-white/10 text-white placeholder-gray-600 focus:border-[#1B4D89]/60 focus:ring-2 focus:ring-[#1B4D89]/20' : 'bg-[#F5F7FA] border-[#E0E4EB] text-gray-900 placeholder-[#6B7280] focus:border-[#1B4D89] focus:ring-2 focus:ring-[#1B4D89]/20';
    ...
    <div className={`min-h-screen ${bg} transition-colors duration-300`}>
      <style>{`
        * { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, Arial, sans-serif; }
        @keyframes toast-in { from{opacity:0;transform:translateX(20px)}to{opacity:1;transform:translateX(0)} } .animate-toast{animation:toast-in 0.3s cubic-bezier(.22,1,.36,1);}
        @keyframes fade-up   { from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)} } .fade-up{animation:fade-up 0.45s cubic-bezier(.22,1,.36,1) both;}
        ...
        @keyframes confirmed-pulse{0%{box-shadow:0 0 0 0 rgba(52,211,153,.4)}70%{box-shadow:0 0 0 12px rgba(52,211,153,0)}100%{box-shadow:0 0 0 0 rgba(52,211,153,0)}}.confirmed-glow{animation:confirmed-pulse 1.5s ease-out 2;}
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        {/* Find a Bus - GRID LAYOUT (2 cards per row) */}
        {tab === 'schedules' && (
                ...
                {is3DVisible
                  ...
                  : <div className="w-full h-[590px] rounded-2xl bg-gradient-to-br from-gray-800/50 to-gray-900/50 flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <p className="text-[#6B7280] font-medium">Loading 3D Seat Map…</p>
                      </div>
                    </div>
                }
